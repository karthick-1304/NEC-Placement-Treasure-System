...
function UploadFeedback() {
  ...
  const [status, setStatus] = useState("not_selected"); // ✅ keep at top
  ...
  const handleSubmit = async (e) => {
    ...
    formData.append("is_selected", status === "selected" ? "1" : "0"); // string → number

    try {
      await uploadFeedback(formData);
      setMessage("Feedback uploaded successfully");
      setFile(null);
      setStatus("not_selected"); // reset dropdown
    } catch (err) {
      console.error(err);
    ...
    <div className="max-w-screen-xl mx-auto px-4 py-10 animate-slide-up">
      <div className="max-w-xl mx-auto bg-dark-800 border border-dark-700 rounded-xl shadow-lg p-8">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          ...
          <div>
            <label className="block text-sm text-dark-300 mb-2">Selection Status</label>
            <select
              value={status}
              onChange={(e) => setStatus(e.target.value)}
              className="w-full px-3 py-2 rounded-lg bg-dark-700 border border-dark-600 text-white focus:outline-none focus:ring-2 focus:ring-primary-500"
            >
        ...
        {message && (
          <p
            className={`mt-4 text-sm ${
              message.includes("success") ? "text-green-400" : "text-rose-400"
            }`}
          >
            {message}
          </p>
